...
| **Site-1.6b** ✓ | `/blog` becomes a living multi-stream feed + RSS                                              | DONE 2026-07-30. `/blog` merges three streams into one reverse-chronological timeline, grouped by month, each entry badged by kind: **articles** (`blog` collection → `/blog/<slug>`), **releases** (the existing `releases` collection, published only → `/downloads#v<version>`, new per-release anchors on the downloads page), and **videos** (new `videos` collection — bare YouTube id, mirroring the `apps` collection's `video` field — reusing `VideoDialog` for the on-page lightbox). One `src/lib/feed.ts` builds the timeline for both the page and a hand-rolled `/blog/rss.xml` (RSS 2.0, no new dependency — same approach as `sitemap.xml.ts`). Stream filter is CSS-only (`:has()` over a radio group), so it filters with JavaScript disabled; default view is everything. 34 entries at ship. Feed styles are `is:inline` so they stay out of the one shared stylesheet the landing page's size budget measures — that budget went 21.45 → 21.37 kB. |
...
| **Site-2.9** ✓ | Launch hygiene (StackScope findings) — security headers, legal pages, cookieless analytics | DONE 2026-07-28. `vercel.json` now sends CSP + X-Content-Type-Options + X-Frame-Options + Referrer-Policy + Permissions-Policy on every route; `/privacy` + `/terms` pages (English-only, footer-linked in all locales, sitemap'd, no phantom hreflang pairs via new `localized` opt-out on Base/Article); Amplitude switched to cookieless (`identityStorage: "none"`, attribution off, IP capture off) so no consent banner is needed — no AMP_* cookies at all. StackScope claim meta added. Partially covers Site-2.4 (the legal `/privacy`; the long-form privacy *posture* page linking the security design docs is still open). |
